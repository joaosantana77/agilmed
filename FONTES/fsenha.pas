...
{  DM.cdsConf.Open;
  ...
  begin
    dtI := now+30;
    dts := SubstBarraponto(DatetoStr(dtI));
  ...
  begin
      ...
      if (dias < 1) then
         showMessage('Versão Demo expirou - Fale com João no (84)87311255)')
      else
      begin
        StatusBar1.Panels[0].Text := 'Versão Demo - Faltam '+FloatToStr(dias)+ ' dias';
      end;
  end;
  if (DM.cdsConfATIVADO.AsInteger = 2) then
  begin
     StatusBar1.Panels[0].Text := 'Versão 2.3';
  end;}
